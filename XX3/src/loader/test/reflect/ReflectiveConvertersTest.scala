...
object ReflectiveConvertersTest {
  ...
  object V {
    ...
    def toV1(u:S,fd:ConvertData,e:Processor#EltBase):V = invoke(u.out,1)
    def toV2(u:S,fd:ConvertData):V = invoke(u.out,2)
    def toV3(u:S,e:Processor#EltBase):V = invoke(u.out,3)
  ...
  object W {
    ...
    def toV1(u:S,fd:ConvertData,e:Processor#EltBase):V = invoke(u.out,1)
    def toV2(u:S,fd:ConvertData):V = invoke(u.out,2)
    def toV3(u:S,e:CtxCore#EltBase):W = invoke(u.out,3)
    def toV4(u:S):W = invoke(u.out,4)
    def toV5(u:T,fd:ConvertData,e:Processor#EltBase):V = invoke(u.out,5)
    def toV6(u:T,fd:ConvertData):V = invoke(u.out,6)
    def toV7(u:T,e:CtxCore#EltBase):W = invoke(u.out,7)
  ...
  class S(val out:PrintWriter) {
    ...
    def toV1(fd:ConvertData,e:Processor#EltBase):V = invoke(1)
    def toV2(fd:ConvertData):V = invoke(2)
    def toV3(e:Processor#EltBase):V = invoke(3)
    def toV4():V = invoke(4)
    def toV5(e:loader.core.CtxCore#EltBase,fd:ConvertData):V = invoke(5) //this also check that order is not important
    def toV6(e:Processor#EltBase,fd:ConvertData):V = invoke(6)
  }
  ...
  class X0(fd:ConvertData) {
    def invoke(out:PrintWriter, n:Int):W = { out.println(s"invoked V.toV$n"); new W(out) }
    def toV3(u:S,e:CtxCore#EltBase):W = invoke(u.out,3)
    def toV4(u:T,e:Processor#EltBase):W    = invoke(u.out,4)
  }
  class X1(fd:ConvertData) extends X0(fd) {
    def toV1(u:S,e:Processor#EltBase):V    = invoke(u.out,1)
    def toV2(u:T,e:CtxCore#EltBase):V = invoke(u.out,2)
  }

  class Y0(fd:ConvertData) {
    def invoke(out:PrintWriter, n:Int):W = { out.println(s"invoked V.toV$n"); new W(out) }
    def fromX(u:S,e:CtxCore#EltBase):W = invoke(u.out,3)
    def fromY(u:T,e:Processor#EltBase):W    = invoke(u.out,4)
  }
  class Y1(fd:ConvertData) extends Y0(fd) {
    def fromX(u:S,e:Processor#EltBase):V    = invoke(u.out,1)
    def fromY(u:T,e:CtxCore#EltBase):V = invoke(u.out,2)
  }
  ...
  @Test class ReflexiveConverterTest extends StandardTester {
    ...
    def apply(file:Solver,out0:PrintWriter) = {
      def checkBase[E<:Processor#EltBase:ClassTag](in:Class[_],cS:Class[_>:T<:S],cV:Class[_>:W<:V],fld:String):Unit = try {
      ...
      def check0[E<:Processor#EltBase:ClassTag](in:Class[_],cS:Class[_>:T<:S],cV:Class[_>:W<:V],n:Int):Unit = checkBase(in,cS,cV,s"toV$n")
      def check(in:Class[_],n:Int) = check0[Processor#EltBase](in,cS,cV,n)
      
      //checking that all converters are found in the source
      for (i <- 1 to 6) check(cS,i)  //5 must fail: CtxCore.Processor#EltBase cannot accept definition.Processor#EltBase
      //checking that all converters are found in the destination
      for (i <- 1 to 4) check(V.getClass,i)
      
      //checking sub-classes for Def#EltBase
      out0.println(Converters(cS, cS, cV, "toV5"))  //None: CtxCore.Def#EltBase cannot accept definition.Def#EltBase
      out0.println(Converters(cS, cS, cV, "toV5")(ClassTag(classOf[loader.core.CtxCore#EltBase])))
      out0.println(Converters(cS, cS, cV, "toV1")(ClassTag(classOf[loader.core.CtxCore#EltBase])))
      
      //checking that all converters are found properly when using derived classes in various places
      out0.println ("---- S to V ----")
      for (i <- 1 to 8) check0[Processor#EltBase](W.getClass,classOf[S],classOf[V],i)  //1,2,4
      out0.println ("---- T to W ----")
      for (i <- 1 to 8) check0[Processor#EltBase](W.getClass,classOf[T],classOf[W],i)  //4,8
      out0.println ("---- S to W ----")
      for (i <- 1 to 8) check0[Processor#EltBase](W.getClass,classOf[S],classOf[W],i)  //4
      out0.println ("---- T to V ----")
      for (i <- 1 to 8) check0[Processor#EltBase](W.getClass,classOf[T],classOf[V],i)     //1,2,4,5,6,8
      out0.println ("---- S to V using CtxProcessor#EltBase ----")
      for (i <- 1 to 8) check0[CtxCore#EltBase](W.getClass,classOf[S],classOf[V],i)  //1,2,3,4
      out0.println ("---- T to W using CtxProcessor#EltBase ----")
      for (i <- 1 to 8) check0[CtxCore#EltBase](W.getClass,classOf[T],classOf[W],i)  //3,4,7,8
      out0.println ("---- S to W using CtxProcessor#EltBase ----")
      for (i <- 1 to 8) check0[CtxCore#EltBase](W.getClass,classOf[S],classOf[W],i)  //3,4
      out0.println ("---- T to V using CtxProcessor#EltBase ----")
      for (i <- 1 to 8) check0[CtxCore#EltBase](W.getClass,classOf[T],classOf[V],i)  //1,2,3,4,5,6,7,8
      ...
      out0.println ("---- S to V using CtxProcessor#EltBase in X1 instancied with fd0 ----")
      checkBase[CtxCore#EltBase](classOf[X1], cS, cW, null) //OK, can find only V3
      checkBase[CtxCore#EltBase](classOf[X1], cS, cV, null) //more than one: V1,V3 (min = V3)
      checkBase[CtxCore#EltBase](classOf[X1], cT, cV, null) //more than one: V1,V2,V3,V4 (min = V2)
      checkBase[CtxCore#EltBase](classOf[X1], cT, cW, null) //no, more than one: V3,V4 (no min)
      out0.println ("---- S to V using CtxProcessor#EltBase in X0 instancied with fd0 ----")
      checkBase[CtxCore#EltBase](classOf[X0], cS, cW, null) //OK, can find only V3
      checkBase[CtxCore#EltBase](classOf[X0], cS, cV, null) //OK, can find only V3
      checkBase[CtxCore#EltBase](classOf[X0], cT, cV, null) //no, more than one: V3,V4 (no min)
      checkBase[CtxCore#EltBase](classOf[X0], cT, cW, null) //no, more than one: V3,V4 (no min)
      out0.println ("---- S to V using Processor#EltBase in X1 instancied with fd0 ----")
      checkBase[Processor#EltBase](classOf[X1], cS, cW, null) //no
      checkBase[Processor#EltBase](classOf[X1], cS, cV, null) //OK, can find only V1
      checkBase[Processor#EltBase](classOf[X1], cT, cV, null) //more than one: V1,V4 (min = V4)
      checkBase[Processor#EltBase](classOf[X1], cT, cW, null) //OK, can find only V4
      out0.println ("---- S to V using Processor#EltBase in X0 instancied with fd0 ----")
      checkBase[Processor#EltBase](classOf[X0], cS, cW, null) //no
      checkBase[Processor#EltBase](classOf[X0], cS, cV, null) //no
      checkBase[Processor#EltBase](classOf[X0], cT, cV, null) //OK, can find only V4
      checkBase[Processor#EltBase](classOf[X0], cT, cW, null) //OK, can find only V4
      ...
      out0.println ("---- S to V using CtxProcessor#EltBase in Y1 instancied with fd0 ----")
      checkBase[CtxCore#EltBase](classOf[Y1], cS, cW, "fromX") //OK, can find only 3
      checkBase[CtxCore#EltBase](classOf[Y1], cS, cV, "fromX") //more than one: 1,3 (min = 3)
      checkBase[CtxCore#EltBase](classOf[Y1], cT, cV, "fromX") //more than one: 1,3 (min = 3)
      checkBase[CtxCore#EltBase](classOf[Y1], cT, cW, "fromX") //OK, can find only 3
      out0.println ("---- S to V using CtxProcessor#EltBase in Y0 instancied with fd0 ----")
      checkBase[CtxCore#EltBase](classOf[Y0], cS, cW, "fromX") //OK, can find only 3
      checkBase[CtxCore#EltBase](classOf[Y0], cS, cV, "fromX") //OK, can find only 3
      checkBase[CtxCore#EltBase](classOf[Y0], cT, cV, "fromX") //OK, can find only 3
      checkBase[CtxCore#EltBase](classOf[Y0], cT, cW, "fromX") //OK, can find only 3
      out0.println ("---- S to V using Processor#EltBase in Y1 instancied with fd0 ----")
      checkBase[Processor#EltBase](classOf[Y1], cS, cW, "fromX") //no
      checkBase[Processor#EltBase](classOf[Y1], cS, cV, "fromX") //OK, can find only 1
      checkBase[Processor#EltBase](classOf[Y1], cT, cV, "fromX") //OK, can find only 1
      checkBase[Processor#EltBase](classOf[Y1], cT, cW, "fromX") //no
      out0.println ("---- S to V using Processor#EltBase in Y0 instancied with fd0 ----")
      checkBase[Processor#EltBase](classOf[Y0], cS, cW, "fromX") //no
      checkBase[Processor#EltBase](classOf[Y0], cS, cV, "fromX") //no
      checkBase[Processor#EltBase](classOf[Y0], cT, cV, "fromX") //no
      checkBase[Processor#EltBase](classOf[Y0], cT, cW, "fromX") //no

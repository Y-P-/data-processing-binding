 ...
 */
trait PrefixPreTreeLike[K, +V, +This <: PrefixPreTreeLike[K, V, This]]
   ...
   */
  type P <: PrefixPreTreeLike.Params[K,V,Repr]
  def params:P
   ...
   */
  protected[this] def newBuilder:PrefixPreTreeLikeBuilder[K,V,Repr]
  
  /** rebuilds this tree with a specific value */
  final def withValue[W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]](value:Option[W])(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]):T = bf.withValue(this, value)
   ...
   */
  def filterView(p: ((K,Repr)) => Boolean): PrefixPreTreeLike[K,V,_] = new WithFilter(p)
   ...
   */
  protected class WithFilter(p: ((K,Repr)) => Boolean) extends PrefixPreTreeLike[K,V,This#WithFilter] {
    type P = Nothing
    def -(key: K): This#WithFilter = ???
    def iterator: Iterator[(K, This#WithFilter)] = self.iterator.filter(p).map(x => (x._1,new x._2.WithFilter(p)))
    protected[this] def newBuilder: PrefixPreTreeLikeBuilder[K,V,This#WithFilter] = ???
    def params: Nothing = ???
    def update[W >: V, T >: This#WithFilter <: PrefixPreTreeLike[K,W,T]](kv: (K, T))(implicit bf: PrefixPreTreeLikeBuilder[K,W,T]): T = ???
    def value: Option[V] = self.value
    def assoc:self.type = self
    /** Rebuilds the view as a true PrefixPreTreeLike. Defaults are again available, and removed keys will fall on default.
     */
    def force:PrefixPreTreeLike.this.Repr = {
   ...
   */
  def update[W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]](kv:(K,T))(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]): T
  
  /** Identical to the previous method, but more than one element is updated.
   */
  def update[W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]](kv1:(K,T),kv2:(K,T),kv:(K,T)*)(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]): T = { val t=kv; update[W,T](kv1 +: kv2 +: t) }
  
  /** Identical to the previous method, but elements are passed through an iterable like rather than a built Seq.
   */
  def update[W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]](kv:GenTraversableOnce[(K,T)])(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]): T =
   ...
   */
  def + [W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]] (kv:(K,T))(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]): T = update[W,T](kv)
   ...
   */
  def + [W>:V,T>:Repr<:PrefixPreTreeLike[K,W,T]] (kv1:(K,T), kv2:(K,T), kvs:(K,T) *)(implicit bf:PrefixPreTreeLikeBuilder[K,W,T]): T =
   ...
   *    @inheritdoc
   */
  def ++[W>:V, T1 >: Repr <: PrefixPreTreeLike[K,W,T1]](xs: GenTraversableOnce[(K, T1)])(implicit bf:PrefixPreTreeLikeBuilder[K,W,T1]): T1 =
    ((repr: T1) /: xs.seq) (_ + _)
...
object PrefixPreTreeLike {
  implicit def toSeq[T<:PrefixPreTreeLike[_,_,T]](t:T):t.SeqView = t.seqView()
   ...
   */
  class Params[K,+V,+T<:PrefixPreTreeLike[K,V,T]] (
   ...
   */
  abstract class Abstract[K, +V, +This <: PrefixPreTreeLike[K, V, This]] extends PrefixPreTreeLike[K, V, This] {
    this:This =>
  }
}

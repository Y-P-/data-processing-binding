...
{ self:This=>
  ...
  def updated [V1>:V,T>:This<:R[K,V1,T]](key:K, tree: T): T = builder[K,V1,T](value,underlying.updated[T](key,tree))
   ...
   */
  def + [V1>:V,T>:This<:R[K,V1,T]](kv: (K, T)): T =
    updated[V1,T](kv._1,kv._2)
  def ++[V1>:V,T>:This<:R[K,V1,T]](xs: GenTraversableOnce[(K, T)]): T =
    xs.foldLeft[T](this)(_+_)
  def mapValues[V1>:V,T<:R[K,V1,T]](f: This => T): T =
    underlying.foldLeft[T](f(this))((t1,t2)=> t1.updated(t2._1,t2._2.mapValues[V1,T](f)))
  def map[K1,V1,T<:R[K1,V1,T]](g:K=>K1, f: This=>T): T =
  ...
  override def filter(kv: ((K, This)) => Boolean):This =
  ...
  def rem (keys:K*): This = {
    ...
    if (m.isEmpty)               this
      ...
      val r = (m.get rem (keys.tail:_*)).repr

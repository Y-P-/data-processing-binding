...
object PrefixLoop {
  ...
  protected abstract class Recur {
     ...
     *  Some trees may contain duplicate nodes ; these may be detected at a hefty processing cost.
     *  In that case, only the first encountered node is actually processed : repeated nodes are at some
     *  point handled the appropriate result.
     *  - for 'degenerate' trees, the first node is fully processed, later nodes get the same result (memory equality)
     *  - for 'infinite' trees, some nodes may have to wait for their result to be available
     ...
     *         This method returns the Result to be processed, or None if no processing is to be done yet.
     *         reprocess may (but this is not ensured) get called twice, once with the option at None, then with it set
     *         to some Result.
                         ...
                         for (child <- nd)
                           nextData(child, ctx) match {
                             ...
                             case d    => val ctx1 = childContext(d,ctx)
                                          ...
                                          if (loop!=null && r!=None) loop(r.get,ctx1)
                           }
                         })
        })

...
trait Core extends definition.Impl {
  ...
  trait DlgBase extends super.DlgBase {dlg:Dlg=>
    ...
    val builder = new EltBuilder {
      def apply[X<:BaseParser with Singleton](parser:X#Parser, userCtx:UCtx[X], parent: Elt, c: Status):Element[X]                                        = new Element(parser,userCtx,dlg,c.key,parent)
      def apply[X<:BaseParser with Singleton](parser:X#Parser, userCtx:UCtx[X], parent: Elt, c: Status, cbks: Cbks*):Element[X] with WithCallbacks        = new ElementCbks(parser,userCtx,dlg,c.key,parent,cbks:_*)
      def apply[X<:BaseParser with Singleton](parser:X#Parser, userCtx:UCtx[X], parent: Elt, c: Status, cb:Cbk, cbks: Cbks*):Element[X] with WithCallback = new ElementCbk(parser,userCtx,dlg,c.key,parent,cb,cbks:_*)
    }
  }  
